...
export default function Scheduling() {
    ...
    async function handleSend() {
        let newMessages = [...messages, { role: 'user', content: prompt }, { role: 'assistant', content: 'Thinking...' }];
        ...
        if (!response.ok) {
            ...
            setMessages([...messages, { role: 'assistant', content: 'Sorry, there was an error processing your request. Please try again.' }]);
            return;
        }
                ...
                {messages.map((message, index) => (
                    message.role === 'assistant' ? (
                        <div key={index} className="flex gap-2">
                            <Image src="/icons/ai-2.svg" className="w-6 h-6" alt="AI" width={22} height={22} />
                            <p>{message.content}</p>
                        </div>
                    ) : (
            ...
            <div className="fixed w-[calc(100%-2rem)] bottom-24">
                <div className="mt-4 relative">
                    ...
                    <div className="flex gap-2 p-2 absolute bottom-2 right-1">
                        <Button variant="secondary" className="w-12 h-12 border border-slate-200 bg-white [&_svg]:!size-6">
                            <Paperclip className="text-slate-500" />
                        </Button>
                        {prompt.length > 0 ? (
                            <Button className="w-12 h-12 [&_svg]:!size-6 hover:bg-blue-500" onClick={handleSend}>
                                <Send />
                            </Button>
                        ) : (
